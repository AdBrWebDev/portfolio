...
export default function About(){
    return(
        <>
            <section class="hero is-medium is-info mt-40" style={{boxShadow: "0 -50px 80px 50px #3e8ed0"}}>
                <div class="hero-body">
            ...
            <div className="grid xl:grid-cols-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-4">
                {gridInterests.map((item, index) => <div key={index} className="bg-white rounded-2xl text-align shadow-xl p-5">
                    <img className={`col-span-${item.spanRow} row-span-${item.spanCol}`} src={`images/${item.img}`} alt={index} />
                </div>)}
            </div>
            </div>
        </>
